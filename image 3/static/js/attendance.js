...
// End of day attendance functionality
let endOfDayModal;
let scheduleData = {};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    endOfDayModal = new bootstrap.Modal(document.getElementById('endOfDayModal'));
    
    // Set today's date in the modal
    document.getElementById('endOfDayDate').value = today;
...
function checkTime() {
    ...
    if (hours === 17 && minutes === 0) { // 5:00 PM
        showEndOfDayModal();
    }
}

// Load schedule data from localStorage or API
...
function showEndOfDayModal() {
    const checklist = document.getElementById('classChecklist');
    checklist.innerHTML = ''; // Clear existing content
    
    // Get today's classes from schedule
    const today = new Date().getDay();
    const dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
    const todayClasses = scheduleData[dayNames[today]] || {};
    
    // Create checklist items for each class
    Object.entries(todayClasses).forEach(([time, subject]) => {
        if (subject && subject !== '' && !subject.includes('Break') && !subject.includes('Lunch')) {
            const checklistItem = document.createElement('div');
            checklistItem.className = 'form-check mb-2';
            checklistItem.innerHTML = `
                <input class="form-check-input" type="checkbox" value="${subject}" id="class_${time}">
                <label class="form-check-label" for="class_${time}">
                    ${time}: ${subject}
                </label>
            `;
...
// Handle saving end of day attendance
document.getElementById('saveEndOfDayAttendance').addEventListener('click', function() {
    const date = document.getElementById('endOfDayDate').value;
    const checkboxes = document.querySelectorAll('#classChecklist input[type="checkbox"]');
